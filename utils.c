                       ...
                       qcdmbool *out_need_more)
{
    ...
    *out_need_more = FALSE;
    ...
    if (inbuf_len < 4) {
        *out_need_more = TRUE;
        printf("dm_decapsulate_buffer: inbuf_len < 4\n");
        return TRUE;
    }
    ...
    for (i = 0; i < inbuf_len; i++) {
        if (inbuf[i] == DIAG_CONTROL_CHAR) {
            ...
            if (i < 3) {
                ...
                *out_used = i + 1;
                printf("dm_decapsulate_buffer: find async control character within 4 bytes.\n");
                return FALSE;
            }

            pkt_len = i;
            printf("dm_decapsulate_buffer: find async control character (0x7e).\n");
            break;
        }
    }
    ...
    if (!pkt_len) {
        *out_need_more = TRUE;
        printf("dm_decapsulate_buffer: coudn't find async control character (0x7e).\n");
        return TRUE;
    }
    ...
    if (!unesc_len) {
        /* Tell the caller to advance the buffer past the control char */
        *out_used = pkt_len + 1;
        printf("dm_decapsulate_buffer: !unesc_len\n");
        return FALSE;
    }

    if (escaping) {
        *out_need_more = TRUE;
        printf("dm_decapsulate_buffer: escaping\n");
        return TRUE;
    }
    ...
    if (crc != pkt_crc) {
        *out_used = pkt_len + 1; /* packet + CRC + 0x7E */
        printf("dm_decapsulate_buffer: CRC failed.\n");
        return FALSE;
    }
